...
function CourseAdmin() {
  ...
  const fetchRegistrations = async () => {
    try {
      ...
      if (activeTab === "partial") {
        url = `${import.meta.env.VITE_API_BASE_URL}/PartialCourse/partial-data`;
      } else {
        url = `${import.meta.env.VITE_API_BASE_URL}/RegisterCourse/registration`;
      }
      ...
      alert("Failed to fetch course registrations");
    }
  };
  ...
  const exportToExcel = () => {
    try {
      ...
      const filename = `Course_Registrations_${activeTab === 'partial' ? 'Partial' : 'Main'}_${timestamp}.csv`;
  ...
  const handleDelete = async (id) => {
    ...
    try {
      let url = "";
      if (activeTab === "partial") {
        url = `${import.meta.env.VITE_API_BASE_URL}/PartialCourse/partial-delete/${id}`;
      } else {
        url = `${import.meta.env.VITE_API_BASE_URL}/RegisterCourse/registration/${id}`;
      }
  ...
  const handleUpdate = async (id, updatedData) => {
    try {
      let url = "";
      if (activeTab === "partial") {
        url = `${import.meta.env.VITE_API_BASE_URL}/PartialCourse/partial-update/${id}`;
      } else {
        url = `${import.meta.env.VITE_API_BASE_URL}/RegisterCourse/registration/${id}`;
      }
  ...
  // Safe resume check function
  const checkResume = (reg) => {
    if (!reg) return false;
    
    console.log("🔍 FULL COURSE REGISTRATION DATA:", reg);
  ...
  const viewResume = async (id, reg) => {
    try {
      console.log("📁 Full course registration data for resume:", reg);
      ...
      const url = `${backendBaseUrl}/upload_courses/${filename}`;
  ...
  const downloadResume = async (id, reg) => {
    try {
      console.log("📥 Course registration data for download:", reg);
  ...
  const renderField = (value, fieldName, isEditing, reg, field) => {
    ...
    if (isEditing) {
      if (fieldName === "previousExperience" || fieldName === "additionalComments") {
        return (
          <textarea 
            defaultValue={value} 
  ...
  // Field configurations for course database
  const getTableFields = () => {
    ...
    if (activeTab === "partial") {
      return [
        ...commonFields,
        { key: "courseName", label: "Course Name", icon: <BookOpen size={14} /> },
  ...
  return (
    <div className="absolute top-31.5 left-66 right-0 bottom-0 p-6 overflow-auto z-10 bg-black/70 text-white">
      {/* Header */}
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-yellow-500 mb-2">Course Registration Admin Panel</h1>
        <p className="text-gray-400">Manage course applications and partial registrations</p>
      </div>
        ...
        <div className="mt-4 p-4 bg-gray-800/80 rounded-lg border border-gray-700">
          <div className="flex justify-between items-center">
            <span className="text-yellow-500 font-semibold text-sm">
              Showing {filteredData.length} records from {activeTab === "partial" ? "Partial" : "Main"} Course Database
            </span>
